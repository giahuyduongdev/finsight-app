...
const serviceConfig = (prefix: string): CircuitBreakerConfig => ({
  failureThreshold:
    getNumberEnv(`${prefix}_CIRCUIT_FAILURE_THRESHOLD`) ??
    getNumberEnv('CIRCUIT_FAILURE_THRESHOLD'),
  resetTimeoutMs:
    getNumberEnv(`${prefix}_CIRCUIT_RESET_TIMEOUT_MS`) ??
    getNumberEnv('CIRCUIT_RESET_TIMEOUT_MS')
})
