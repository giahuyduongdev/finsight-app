...
const getRefreshRotationGraceMs = () => {
  ...
  if (
    !Number.isInteger(rawGraceSeconds) ||
    rawGraceSeconds < 0 ||
    rawGraceSeconds > REFRESH_ROTATION_GRACE_MAX_SECONDS
  ) {
    return 10 * 1000
  }

  return rawGraceSeconds * 1000
}
...
const createRefreshTokenRecord = async ({
  ...
  await RefreshTokenModel.findOneAndUpdate(
    { token: refreshTokenHash },
    {
      ...
      isRevoked: false,
      tokenFamilyId,
      ...(rotatedFromToken ? { rotatedFromToken } : {})
    },
    { upsert: true, new: true, session }
  )

  return { refreshToken, refreshTokenHash, tokenFamilyId }
}

const revokeRefreshTokenFamilyForReplay = async (
  tokenFamilyId: string,
  tokenHash: string
) => {
  const replayDetectedAt = new Date()
  ...
  logger.warn('[APP:Auth] Refresh token replay detected', {
    tokenFamilyId,
    tokenDigestPreview: tokenHash.slice(0, 12)
  })
}
...
export const refreshTokenService = async (
  ...
  if (refreshToken.isRevoked) {
    ...
    const isGraceEligible =
      ...
      !!refreshToken.reuseGraceUntil &&
      refreshToken.reuseGraceUntil >= now

    if (isGraceEligible) {
      const activeReplacement = await RefreshTokenModel.findOne({
        token: refreshToken.replacedByToken,
        tokenFamilyId: refreshToken.tokenFamilyId,
        rotatedFromToken: tokenHash,
        isRevoked: false,
        expiresAt: { $gt: now }
      })
    ...
    if (
      ...
      await revokeRefreshTokenFamilyForReplay(
        refreshToken.tokenFamilyId,
        tokenHash
      )
    }
  ...
  try {
    await session.withTransaction(async () => {
      const replacement = await createRefreshTokenRecord({
        ...
        tokenFamilyId: refreshToken.tokenFamilyId || crypto.randomUUID(),
        rotatedFromToken: tokenHash,
        session
      })
      const now = new Date()
      const reuseGraceUntil = new Date(
        now.getTime() + getRefreshRotationGraceMs()
      )
      const rotatedToken = await RefreshTokenModel.findOneAndUpdate(
        ...
        },
        {
          ...
          rotatedAt: now,
          reuseGraceUntil,
          replacedByToken: replacement.refreshTokenHash,
          tokenFamilyId: replacement.tokenFamilyId
        },
        { new: true, session }
      )
      ...
      nextRefreshToken = replacement.refreshToken
    })
  } finally {
    await session.endSession()
  }
...
export const createRefreshToken = async (
  ...
  const { refreshToken } = await createRefreshTokenRecord({ userId, userAgent })
