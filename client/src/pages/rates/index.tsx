...
const RatesPage = () => {
  ...
  const preferredCurrency = useTypedSelector((state) => state.auth.user?.preferredCurrency) || 'VND'
  ...
  return (
    <PageLayout title="Exchange Rates" subtitle="Real-time currency prices and converter">
        ...
        <div className="flex items-center justify-between bg-white dark:bg-card p-4 rounded-xl border border-gray-100 dark:border-border">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground">
                  {isConnected ? 'Connected to Live Feed' : 'Offline - Using Cached Rates'}
                </span>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-4">
            ...
            <div className="text-right">
              ...
              <p className="text-sm font-mono">{rates?.updatedAt ? new Date(rates.updatedAt).toLocaleTimeString() : '---'}</p>
            </div>
          </div>
        </div>
            ...
            <Card className="!shadow-none border-1 border-gray-100 dark:border-border">
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">Rates relative to {preferredCurrency}</p>
                </div>
              ...
              <CardContent>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-100 dark:border-border text-muted-foreground">
                        <th className="h-10 px-2 text-left font-medium">Currency</th>
                        <th className="h-10 px-2 text-right font-medium">Exchange Rate</th>
                        <th className="h-10 px-2 text-right font-medium">Value in {preferredCurrency}</th>
                      </tr>
                    </thead>
                      ...
                      {!rates ? (
                        [1, 2, 3, 4, 5].map((i) => (
                          <tr key={i}>
                            ...
                            <td className="p-4"><Skeleton className="h-4 w-32 ml-auto" /></td>
                          </tr>
                        ))
                      ) : (
                        currencyList.map(([code, rateFromVND]) => {
                          // Tỉ giá của PreferredCurrency so với VND
                          const prefRateFromVND = rates.rates[preferredCurrency] || 1
                          ...
                          return (
                            <tr key={code} className="hover:bg-gray-50/50 dark:hover:bg-white/5 transition-colors group">
                              <td className="p-4">
                                <div className="flex items-center gap-2">
                                  <Badge variant="outline" className="font-mono">{code}</Badge>
                                  {code === preferredCurrency && <Badge className="bg-primary/10 text-primary border-none text-[10px] h-4">Base</Badge>}
                                  {rates.usedCurrencies?.includes(code) && code !== preferredCurrency && (
                                    <Badge variant="secondary" className="text-[10px] h-4">In Use</Badge>
                                  )}
                                </div>
                              </td>
                              <td className="p-4 text-right font-mono">
                                1 {preferredCurrency} = {crossRate.toFixed(4)} {code}
                              </td>
                              <td className="p-4 text-right font-bold">
                                {formatCurrency(valueInPref, { currency: preferredCurrency })}
                              </td>
                            </tr>
                          )
                        })
                ...
                <div className="mt-6 p-4 bg-blue-50/50 dark:bg-blue-900/10 rounded-lg flex gap-3 border border-blue-100/50 dark:border-blue-900/20">
                  ...
                  <p className="text-xs text-blue-700 dark:text-blue-400 leading-relaxed">
                    These are mid-market exchange rates for informational purposes. Actual bank rates for buying or selling may differ slightly due to spreads and fees.
                  </p>
                </div>
              </CardContent>
            </Card>
